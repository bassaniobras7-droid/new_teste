...
path = Path('src/src/excel_writer.py')
text = path.read_text(encoding='utf-8')
start = text.index('def write_aditivos_distrato_sheet')
end = text.index('def write_client_sheet', start)
# new function:
new_func = '''def write_aditivos_distrato_sheet(sheet, summary_normal, summary_distratado, price_data, bold_font, regular_font, header_fill, currency_format, accounting_format, bold_white_font=None, regular_white_font=None, client_normal=None, client_distratado=None):
    ...
    def write_section(left_rows, right_rows, start_row, section_title):
        ...
        return start_row + 1, current_row - 1, current_row
    ...
    sheet.page_setup.printArea = f'A1:M{subtotal_row}'
    sheet.sheet_view.view = 'pageBreakPreview'
'''
text = text[:start] + new_func + text[end:]
path.write_text(text, encoding='utf-8')
print('write_aditivos_distrato_sheet function replaced')
